...
from sendRecivRsa.send import send_message
from sendRecivRsa.receiv import reciev
listening_port = 8080
max_conn = 5
buffer_size = 4096
global_port = 9999

def proxy_server(webserver, port, conn, client_data_str, addr):
    method = client_data_str.split(" ")[0]
    try:
        if (method == "CONNECT"):
            https_request(webserver, port, conn, client_data_str, addr)
        else:
            # we create a new socket that will send the request for the client
            http_request(webserver, port, conn, client_data_str, addr)
...
def conn_string(conn, client_data: bytes, addr,global_port):
    """
    ...
    :return:
    """
    client_data_str = client_data.decode("latin-1")
    # print("CLIENT DATA: ", client_data)
    try:
        first_line = client_data_str.split('\n')[0]
        ...
        print(f'CLIENT DATA ICI : {client_data}')
        #todo ICI CALL SEND_MESSAGE
        proxy_server(webserver, port, conn, client_data_str, addr)
    except Exception as e:
        print("ERRR", e)
        sys.exit(1)


def start(global_port=None):
    try:
    ...
    while True:
        try:
            ...
            global_port -=1
            start_new_thread(conn_string, (conn, client_data, addr,global_port))
